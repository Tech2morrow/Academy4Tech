...
export async function onRequestPost({ request, env }) {
  ...
  if (origin) {
    ...
    } catch { return respond({ error: 'Cross-site form submissions are not accepted.' }, 403); }
  }

  let input;
  try { input = await readPayload(request); } catch { return respond({ error: 'The submitted form could not be read.' }, 400); }

  if (clean(input.company, 200)) return respond({ ok: true }, 201);
  const startedAt = Number(input.startedAt);
  const elapsed = Date.now() - startedAt;
  if (!Number.isFinite(startedAt) || elapsed < 3000 || elapsed > 7200000) return respond({ error: 'Please refresh the page and complete the form again.' }, 400);
  ...
  const interests = Array.isArray(input.interests) ? [...new Set(input.interests.map(value => clean(value, 40)).filter(value => allowedInterests.has(value)))] : [];
  ...
  if (!allowedRoles.has(role) || !allowedAges.has(ageGroup) || !allowedLevels.has(experienceLevel) || !allowedFormats.has(preferredFormat)) return respond({ error: 'Please complete all required selections.' }, 400);
  if (!interests.length) return respond({ error: 'Choose at least one learning interest.' }, 400);
  if (input.guardianConsent !== 'yes' || input.privacyConsent !== 'yes') return respond({ error: 'Both consent confirmations are required.' }, 400);
  if (ageGroup === 'under-16' && role !== 'parent-guardian') return respond({ error: 'A parent or guardian must submit for a learner under 16.' }, 400);
  if (!env.STUDENT_INTEREST_DB) return respond({ error: 'The interest form is being connected. Please try again later.' }, 503);

  try {
    await env.STUDENT_INTEREST_DB.prepare(`INSERT INTO course_interest (id, created_at, name, email, submitter_role, age_group, country, experience_level, interests, preferred_format, message, consent_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), new Date().toISOString(), name, email, role, ageGroup, country || null, experienceLevel, JSON.stringify(interests), preferredFormat, message || null, '2026-08-11').run();
    return respond({ ok: true }, 201);
  } catch (error) {
    console.error('Course interest insert failed', error);
    return respond({ error: 'The form could not be saved. Please try again later.' }, 500);
  }
}
